...
-- Query para encontrar los materiales en un proyecto
SELECT m.descripcion MaterialUsado
FROM materiales m, entregan e, proyectos p
WHERE m.clave = e.clave 
AND e.numero = p.numero
AND p.denominacion = "Restauración de instalaciones del CEA"

-- Query para encontrar el precio total de cada entrega
SELECT m.descripcion Material, pr.razonsocial Proveedor, p.denominacion Proyecto, e.cantidad*(m.precio+m.impuesto) PrecioTotal, e.fecha Fecha
FROM materiales m, entregan e, proveedores pr, proyectos p
WHERE m.clave = e.clave
AND e.rfc = pr.rfc
AND e.numero = p.numero

-- Query para encontrar los proyectos a los que un provedoor entrega
SELECT p.denominacion Proyecto
FROM proveedores pr, entregan e, proyectos p
WHERE pr.rfc = e.rfc
AND e.numero = p.numero
AND pr.razonsocial = "Oviedo"
